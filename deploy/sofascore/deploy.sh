...
BREAKER_CALLS=0
BREAKER_RESULT="-"
# Бюджет вызовов — по числу скоупов батча (#1248 ступень 1): скоупы паркуются РАЗНЫМИ
# волнами (один ещё работает, когда соседи уже встали в очередь за слотом), и одного вызова
# второй волне не хватило бы — шаг снова ждал бы сам себя до потолка. Больше, чем скоупов,
# не нужно: удачный вызов гасит ВСЕ припаркованные разом, а погасить скоуп можно лишь один
# раз. Поэтому бюджет считается от той же ручки, которую читает DAG
# (SOFASCORE_HISTORY_BATCH_SIZE из env-файла контура), а не от литерала: при batch=4 литерал
# 3 оставил бы четвёртую волну без вызова. Бюджет тратят ТОЛЬКО удачные вызовы: сбой или
# зависание съедает его целиком и повторов в этом шаге не будет (Sol круг 3 #1245) —
# внешний `timeout` убивает КЛИЕНТА docker exec, а процесс в контейнере переживает его
# (moby#9098) и мог бы тронуть задачу уже после возврата слотов пула.
BREAKER_MAX="${SOFASCORE_DEPLOY_BREAKER_MAX:-${SOFASCORE_HISTORY_BATCH_SIZE:-3}}"
# Мусор в env-файле не должен ни ронять шаг drain арифметикой, ни тихо обнулять бюджет:
# рамка та же, что у batch в DAG (1..64). Строкового сравнения мало — "00" и "000" состоят
# из цифр, но дают нулевой бюджет и ломателя не позвали бы ни разу; длину проверяем до
# сравнения, иначе слишком длинное число сломает сам `test`.
case "$BREAKER_MAX" in ''|*[!0-9]*) BREAKER_MAX=3 ;; esac
if ! { [ "${#BREAKER_MAX}" -le 2 ] && [ "$BREAKER_MAX" -ge 1 ] && [ "$BREAKER_MAX" -le 64 ]; }; then
  BREAKER_MAX=3
fi
...
break_deadlock() {
  ...
  if [ "$brc" = 0 ]; then
    ...
    BREAKER_RESULT=сбой
    # Не дозвонились — бюджет съеден целиком: повторять сбойный или зависший вызов внутри
    # одного шага нельзя, см. комментарий у BREAKER_MAX.
    BREAKER_CALLS="$BREAKER_MAX"
