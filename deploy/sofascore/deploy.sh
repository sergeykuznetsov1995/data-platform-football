...
# второй волне не хватило бы — шаг снова ждал бы сам себя до потолка. Бюджет тратят ТОЛЬКО
# удачные вызовы: сбой или зависание съедает его целиком и повторов в этом шаге не будет
# (Sol круг 3 #1245) — внешний `timeout` убивает КЛИЕНТА docker exec, а процесс в контейнере
# переживает его (moby#9098) и мог бы тронуть задачу уже после возврата слотов пула.
BREAKER_MAX="${SOFASCORE_DEPLOY_BREAKER_MAX:-3}"
