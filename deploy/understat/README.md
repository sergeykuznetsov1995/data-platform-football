...
Метка берётся из метабазы (`now()`) ПОСЛЕ записи всех файлов. Затем (до 7 мин, опрос раз в 20 с):
для обоих DAG `dag.has_import_errors = f` и `dag.last_parsed_time > метка + 60 с` (запас больше
таймаута разбора файла: разбор, начатый до окончания записи, не засчитывается); `import_error where filename like '%understat%'` = 0; затем
cmp всех файлов Understat в бою с master пустой. Успех → `understat-accepted` ← SHA.

Состояние для отката (`understat-inflight`, `accepted-before`, `files`) пишется до первой записи в бой;
не записалось — бой не трогается. Не записался `understat-accepted` после успешной приёмки → 🆘 и выключатель.
