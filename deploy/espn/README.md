...
- **Зависший POSTPONED/SUSPENDED**, по которому core ответил финалом, пишется с summary: счёт —
  ...
  чтении издания — ошибка своего турнира, не волны. Алертов в Telegram из DAG нет. **Открыто:**
  красный турнир при зелёной волне (≤ 20 %) виден только в итоге `wave_summary` (лог и XCom);
  сторож `espn_stall_watch.py` ловит паузу и общий простой, но не отдельный красный турнир —
  тревога на красный турнир в #1504 не сделана (решение — у Fable).
