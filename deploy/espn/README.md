...
| `plan_wave` | издания из кэша → статусы дня `all/scoreboard` за вчера и сегодня UTC (`force_refresh`) → сверка с `bronze.espn_match` → список турнир-сезонов с работой; в волну 00 — ещё проверка зависших POSTPONED/SUSPENDED старше 3 суток | 2 |
...
- **Пропавший матч — не авария.** Известный незавершённый матч, которого нет в ответе его дня:
  ...
  (kickoff прежний: статус даты не несёт; известный незавершённый статус из ответа пишется в
  строку). Помеченный матч повторно не проверяется каждой волной. Больше
...
- **Цвет.** Упавший турнир красный с первой ошибкой (`<Тип>: <сообщение>`), соседи публикуются.
  ...
  турнир, без повторов. Алертов в Telegram из DAG нет: реакция на сбой — красный турнир + сторож
  `espn_stall_watch.py`.
- **Время волны** пишется в лог `wave_summary`; p95 (конец − старт) ≤ 60 мин за 3 суток меряется
  по журналу `iceberg.ops.espn_request_journal_v1` (min/max `requested_at` на `run_id`) после
  первого живого прогона — приёмка вехи 1 (#1505). Оценка пиковой волны: 2 статуса дня + 161
