...
# Суточная защёлка — только после УСПЕШНОЙ отправки: иначе неудачный хук глушил бы
# алерт до завтра, а окно терялось бы молча (ревью Sol, #1155).
if "$TG_HOOK" "🟢 FotMob: окно доставки ОТКРЫТО ($(date -u +%H:%M) UTC, закроется к 23:55 UTC). Доставка идёт сама по cron; вмешаться: автомат auto_deliver.sh (лог $LOG)"; then
  touch "$FLAG"
else
  echo "хук алерта $TG_HOOK вернул ошибку — защёлку не ставлю, повторю следующим тиком" >&2
  exit 3
fi
