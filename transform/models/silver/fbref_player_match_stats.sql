...
-- Ported from dags/sql/silver/fbref_player_match_stats.sql (Tier-2 dbt-trino pilot).
...
--
-- Source:
--   {{ source('bronze', 'fbref_match_player_stats') }}
--
-- Deduplication:
...
WITH src AS (
    ...
    FROM {{ source('bronze', 'fbref_match_player_stats') }}
)
