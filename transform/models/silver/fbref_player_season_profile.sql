...
-- Ported from dags/sql/silver/fbref_player_season_profile.sql (Tier-2 dbt-trino pilot).
...
WITH s AS (
    ...
    FROM {{ source('bronze', 'fbref_player_stats') }}
),
...
sh AS (
    ...
    FROM {{ source('bronze', 'fbref_player_shooting') }}
),
...
pl AS (
    ...
    FROM {{ source('bronze', 'fbref_player_playingtime') }}
),
...
mi AS (
    ...
    FROM {{ source('bronze', 'fbref_player_misc') }}
)
