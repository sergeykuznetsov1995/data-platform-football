...
-- Ported from dags/sql/silver/fbref_team_season_profile.sql (Tier-2 dbt-trino pilot).
...
WITH s AS (
    ...
    FROM {{ source('bronze', 'fbref_team_stats') }}
),
...
sh AS (
    ...
    FROM {{ source('bronze', 'fbref_team_shooting') }}
),
...
mi AS (
    ...
    FROM {{ source('bronze', 'fbref_team_misc') }}
),
...
pt AS (
    ...
    FROM {{ source('bronze', 'fbref_team_playingtime') }}
),
...
gk_dedup AS (
    ...
    FROM {{ source('bronze', 'fbref_keeper_keeper') }}
),
