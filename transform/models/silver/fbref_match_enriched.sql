...
-- Ported from dags/sql/silver/fbref_match_enriched.sql (Tier-2 dbt-trino pilot).
...
ts AS (
    ...
    FROM {{ source('bronze', 'fbref_match_team_stats') }}
),
...
events_dedup AS (
    ...
    FROM {{ source('bronze', 'fbref_match_events') }}
),
...
lineups_dedup AS (
    ...
    FROM {{ source('bronze', 'fbref_lineups') }}
),
