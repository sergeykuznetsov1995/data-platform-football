...
def _filter_new_match_ids(
    ...
    if manifest_ids is not None:
        if season_str != season_short:
            legacy_partition = _existing_complete_capture_ids(league, season_str)
...
def _run_player_ratings(
    ...
    # Preserve the integer start-year contract while keeping #913 single-year
...
def _run_match_capture(
    ...
    try:
        ...
        if not match_ids:
            match_ids = _resolve_match_ids_from_bronze(league, season_str, None)
    except Exception as exc:
...
def main():
    ...
    if 'INT-World Cup' in leagues:
        ...
        if len(leagues) > 1:
            ...
            leagues = [league for league in leagues if league != 'INT-World Cup']
            ...
            if _wc_season is None or int(args.season) != int(_wc_season):
                logger.info(
                    "INT-World Cup: keeping explicit --season %s (configured "
                    "edition, WC1 historical backfill).", args.season)
