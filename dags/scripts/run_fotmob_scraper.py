...
# Короткий возврат скоупа ради матча, по которому источник ответил «данных нет»
# (#1450): дыра считается подтверждённой после двух ранов с промежутком не меньше
# двух часов (или после трёх любых); спустя трое суток от начала матча короткого
# возврата ради него нет. Это потолок коротких ВОЗВРАТОВ СКОУПА, не запросов на
# матч — см. docstring `_unconfirmed_gap_matches`.
...
def _unconfirmed_gap_matches(
    ...
    Потолок ограничивает КОРОТКИЕ ВОЗВРАТЫ СКОУПА — не больше трёх на матч-кандидат,
    пока он не подтверждён и моложе 72 ч, — а не запросы на матч. При каждом
    коротком возврате `sync_match_payloads` (service.py:2487) переспрашивает все
    матчи скоупа без успешных деталей, включая уже подтверждённые дыры. Верхняя
