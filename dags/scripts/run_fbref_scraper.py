...
def main():
    ...
    # #920 bridge (generalized Phase 3: any single_year tournament, not just
    # INT-World Cup): tournaments must never inherit the club-formula season
    # (July 2026 -> 2025). A 2025-labelled WC run both MISLABELS the
    ...
    # call cannot carry two seasons, so tournaments are dropped with a
    # warning (scrape them in dedicated calls), mirroring run_whoscored_scraper.
    ...
    if _tournaments and len(leagues) > 1:
        ...
        if not leagues:
            logger.warning("No leagues left after dropping tournaments; "
                           "exiting 0.")
        ...
        if _t_season is None:
            logger.warning(
                f"{_t_league} is out of its tournament window "
