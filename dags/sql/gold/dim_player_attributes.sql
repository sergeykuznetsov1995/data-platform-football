...
--   - FotMob-side bridge: per canonical_id берётся latest season (по
--     xref_player.season DESC) — атрибуты time-invariant, но xref хранит
--     row per (source, source_id, season).
--   - Attributes published per source: born_year_fbref / dob_fotmob /
--     nationality_fbref / nationality_fotmob / height_cm_fotmob /
--     foot_fotmob / contract_end_fotmob. Никаких "правильных" значений —
--     потребитель решает сам. contract_end — slowly-changing, snapshot
--     отражает "as-of-latest-ingest" (не historical, см. issue #11 для
--     timeline-варианта market_value/contract_end).
--
...
SELECT
    ...
    COALESCE(fb.player_name, fm.player_name)           AS player_name_canonical,
