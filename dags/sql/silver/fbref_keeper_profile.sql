...
--   Blank source IDs are resolved by silver.fbref_player_identity before any
--   window or join. ROW_NUMBER() then partitions by the resolved identity:
--   ROW_NUMBER() OVER (PARTITION BY resolved_player_id, squad, league, season
...
s_identified AS (
    ...
    INNER JOIN identity i
        ...
        AND i.normalized_team = regexp_replace(
            regexp_replace(normalize(lower(TRIM(COALESCE(b.squad, ''))), NFD), '\p{Mn}+', ''),
            '[^a-z0-9]+', ''
        )
        AND (
...
k_identified AS (
    SELECT b.*, i.player_id AS resolved_player_id
    FROM iceberg.bronze.fbref_keeper_keeper b
    INNER JOIN identity i
        ON  i.league = b.league
        AND i.season = CASE
        ...
        END
        AND i.normalized_name = regexp_replace(regexp_replace(normalize(lower(TRIM(b.player)), NFD), '\p{Mn}+', ''), '[^a-z0-9]+', '')
        AND i.normalized_team = regexp_replace(regexp_replace(normalize(lower(TRIM(COALESCE(b.squad, ''))), NFD), '\p{Mn}+', ''), '[^a-z0-9]+', '')
...
FROM s
LEFT JOIN k
    ON  s.resolved_player_id = k.resolved_player_id
    AND s.squad     = k.squad
