...
# Суточные потолки — предохранитель от разгона в цикл, а НЕ регулятор вежливости.
# Вежливость задаёт FOTMOB_DAILY_REQUESTS_PER_MINUTE — её НЕ трогаем.
#
# Подняты 2026-07-31 (ревизия этапа A плана полного покрытия). Почему:
#   прежние 10 000 req/сут были в 8,6 раза НИЖЕ того, что уже разрешает сам rate-limit
#   (60 rpm × 1440 мин = 86 400 req/сут) — потолок резал не сайт, а нас.
#   Замеры: лёгкий refresh 42,5 req/турнир (волна 30.07: 893 req / 21 турнир),
#   полная волна ~448 req/турнир. Ротация всего каталога (494 турнира) — это
#   ~21 000 req в лёгком режиме и ~221 000 в полном, то есть ×2,1 и ×22 к старому
#   потолку; заявленный «цикл ротации 2–3 дня» при 10 000 req/сут = 22 суток.
#   По байтам ~23 МиБ/турнир → ~11,3 ГиБ на полную ротацию.
# Трафик FotMob прямой (max_proxy_bytes = 0, прокси в контуре нет) — в деньгах ноль.
#
# ⚠️ МЕНЯТЬ ТОЛЬКО МЕЖДУ ВОЛНАМИ: dag_ingest_fotmob.py:145-150 сверяет минтованный
# бюджет DagRun с этими константами при КАЖДОМ импорте, а задачи Airflow — отдельные
# процессы. Правка на ходу даёт «daily transport budget mismatch» и убивает волну.
FOTMOB_DAILY_MAX_REQUESTS = 80_000
FOTMOB_DAILY_MAX_DIRECT_MIB = 5_120
