...
# 2026-08-10: потолки подняты до продовых при доставке #1149 в изолированный контур.
# Прежние (10_000/512, 15_000/1_024, 10_000/512) рассчитаны на когорту из 21 турнира;
# автоматический каталог расширяет её до ~450 мужских турниров при замеренных
# ~44 запросах на турнир (дневной ран 09.08: 928 запросов на 21 турнир), то есть
# полный оборот стоит ~20k запросов и упирался бы в 'request budget exceeded'.
# rpm НЕ поднимаем — темп обращений к источнику прежний; растёт только длина рана,
# а фоновые лэйны всё равно ограничены дедлайном 13:30 UTC (build_child_conf ниже).
_LANE_CAPS = {
