...
def scrape_schedule(**context) -> Dict[str, Any]:
    """
    Scrape match schedules from FotMob.
    ...
    logger.info(f"Starting FotMob schedule scrape: leagues={leagues}, season={season}")

    results = {'tables': [], 'rows': 0, 'errors': []}

    try:
        with FotMobSeleniumScraper(
            leagues=leagues,
            seasons=[season],
            headless=True,
            use_xvfb=True,
...
def scrape_team_stats(**context) -> Dict[str, Any]:
    """
    Scrape team statistics from FotMob.
    ...
    logger.info(f"Starting FotMob team stats scrape: leagues={leagues}, season={season}")
    ...
    try:
        with FotMobSeleniumScraper(
            ...
            for league in leagues:
                try:
                    df = scraper.read_team_season_stats(league, season)
                    if df is not None and not df.empty:
                        table_path = scraper.save_to_iceberg(
                            df=df,
...
def scrape_player_stats(**context) -> Dict[str, Any]:
    """
    Scrape player statistics from FotMob.
    ...
    logger.info(f"Starting FotMob player stats scrape: leagues={leagues}, season={season}")
    ...
    try:
        with FotMobSeleniumScraper(
            ...
            for league in leagues:
                try:
                    df = scraper.read_player_season_stats('goals', league, season)
                    if df is not None and not df.empty:
                        table_path = scraper.save_to_iceberg(
                            df=df,
...
with DAG(
    ...
    catchup=False,
    tags=['scraping', 'fotmob', 'bronze', 'football'],
    max_active_runs=1,
    params={
        'leagues': LEAGUES,
