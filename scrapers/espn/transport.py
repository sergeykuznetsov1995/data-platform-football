"""Direct-only native HTTPS transport for ESPN JSON endpoints (#1500).

Where and when a request goes is decided by ``TransportGate`` (one gate per
VM): one permit per HTTP attempt, the origin chosen per cluster.  A 403
closes the origin and defers the request (``OriginBlocked``) instead of
retrying it on another origin; 408/425/429/5xx and timeouts are retried
within ``max_attempts``, each retry with a new permit.  Every request leaves
one ledger entry (address, status, attempts, bytes, encoding, latency, step,
lane); ``scrapers/espn/journal.py`` persists them.  Raw bodies are written by
a background ``RawWriteQueue``: call ``flush()`` at the end of a batch.
"""

from __future__ import annotations

import json
import logging
import math
import os
import re
import threading
import time
import zlib
from datetime import datetime, timezone
from email.utils import parsedate_to_datetime
from typing import Callable, Mapping, Optional
from urllib.parse import urlsplit, urlunsplit

import requests
from urllib3.exceptions import HTTPError as Urllib3HTTPError
from urllib3.exceptions import ReadTimeoutError as Urllib3ReadTimeoutError

from .gate import Permit, TransportGate
from .raw_store import (
    EspnRawStore,
    RawJsonRecord,
    RawStoreError,
    RawTargetCorrupt,
    RawTargetNotFound,
    RawWriteQueue,
)
from .transport_contracts import (
    AllOriginsBlocked,
    AmbientProxyError,
    CanonicalTarget,
    DailyCapExceeded,
    DEFAULT_CONNECT_TIMEOUT,
    DEFAULT_MAX_ATTEMPTS,
    DEFAULT_READ_TIMEOUT,
    DEFAULT_RESPONSE_CAP_BYTES,
    DirectTransportError,
    EndpointType,
    EspnTransportError,
    FetchResult,
    HttpStatusError,
    InvalidJsonError,
    LaneClosed,
    OriginBlocked,
    Params,
    RequestLedgerEntry,
    ResponseTooLarge,
    RetryExhausted,
    _nonnegative_int,
    canonicalize_target,
    normalize_transport_origin,
)


logger = logging.getLogger(__name__)

RETRYABLE_HTTP_STATUSES = frozenset({408, 425, 429})
PROXY_VARIABLES = frozenset({"HTTP_PROXY", "HTTPS_PROXY", "ALL_PROXY"})


def _transport_origin(url: str) -> str:
    parsed = urlsplit(url)
    return normalize_transport_origin(
        urlunsplit((parsed.scheme, parsed.netloc, "", "", ""))
    )


def _on_origin(url: str, origin: str) -> str:
    parsed = urlsplit(url)
    return urlunsplit(
        (parsed.scheme, urlsplit(origin).netloc, parsed.path, parsed.query, "")
    )


class _ReadLimitExceeded(Exception):
    def __init__(self, direct_bytes: int) -> None:
        self.direct_bytes = direct_bytes


class _ResponseEncodingError(Exception):
    def __init__(self, direct_bytes: int, cause: Exception) -> None:
        self.direct_bytes = direct_bytes
        self.__cause__ = cause


class _ResponseReadTimeout(Exception):
    def __init__(self, direct_bytes: int) -> None:
        self.direct_bytes = direct_bytes


class _ResponseReadFailure(Exception):
    def __init__(self, direct_bytes: int, error_type: str) -> None:
        self.direct_bytes = direct_bytes
        self.error_type = error_type


class _Attempts:
    """Per-request bookkeeping for the ledger entry."""

    def __init__(self, requested_at: str) -> None:
        self.requested_at = requested_at
        self.count = 0
        self.status: Optional[int] = None
        self.direct_bytes = 0
        self.origin: Optional[str] = None
        self.step: Optional[int] = None
        self.content_encoding: Optional[str] = None
        self.origins: list[tuple[str, Optional[int]]] = []

    def start(self, permit: Permit) -> None:
        self.count += 1
        self.origin = permit.origin
        self.step = permit.step
        self.status = None
        self.origins.append((permit.origin, None))

    def finish(self, status: Optional[int]) -> None:
        self.status = status
        if self.origins:
            self.origins[-1] = (self.origins[-1][0], status)


class EspnHttpClient:
    """Cache-first HTTPS client; pace, origins and lanes come from the gate."""

    def __init__(
        self,
        raw_store: Optional[EspnRawStore] = None,
        *,
        gate: TransportGate,
        raw_store_uri: Optional[str] = None,
        session: Optional[requests.Session] = None,
        connect_timeout: float = DEFAULT_CONNECT_TIMEOUT,
        read_timeout: float = DEFAULT_READ_TIMEOUT,
        response_cap_bytes: int = DEFAULT_RESPONSE_CAP_BYTES,
        max_attempts: int = DEFAULT_MAX_ATTEMPTS,
        sleep_fn: Callable[[float], None] = time.sleep,
        monotonic_fn: Callable[[], float] = time.monotonic,
        utcnow_fn: Callable[[], datetime] = lambda: datetime.now(timezone.utc),
        environ: Optional[Mapping[str, str]] = None,
    ) -> None:
        environment = os.environ if environ is None else environ
        proxy_names = sorted(
            key
            for key, value in environment.items()
            if key.upper() in PROXY_VARIABLES and str(value).strip()
        )
        if proxy_names:
            raise AmbientProxyError(
                "Ambient proxy variables are forbidden for ESPN transport: "
                + ", ".join(proxy_names)
            )
        if raw_store is not None and raw_store_uri is not None:
            raise ValueError("provide raw_store or raw_store_uri, not both")
        if not isinstance(gate, TransportGate):
            raise TypeError("gate must be a TransportGate")
        self.gate = gate
        self.lane = gate.lane
        self.raw_store = (
            raw_store
            or (EspnRawStore.from_uri(raw_store_uri) if raw_store_uri else None)
            or EspnRawStore.from_env()
        )
        self.raw_writer = RawWriteQueue(self.raw_store)
        self.connect_timeout = float(connect_timeout)
        self.read_timeout = float(read_timeout)
        self.response_cap_bytes = _nonnegative_int(
            response_cap_bytes, "response_cap_bytes"
        )
        self.max_attempts = _nonnegative_int(max_attempts, "max_attempts")
        if (
            min(
                self.connect_timeout,
                self.read_timeout,
                self.response_cap_bytes,
                self.max_attempts,
            )
            <= 0
        ):
            raise ValueError("ESPN transport bounds must all be positive")
        hard_bounds = {
            "connect_timeout": (self.connect_timeout, DEFAULT_CONNECT_TIMEOUT),
            "read_timeout": (self.read_timeout, DEFAULT_READ_TIMEOUT),
            "response_cap_bytes": (
                self.response_cap_bytes,
                DEFAULT_RESPONSE_CAP_BYTES,
            ),
        }
        for name, (value, hard_maximum) in hard_bounds.items():
            if value > hard_maximum:
                raise ValueError(f"{name} cannot exceed {hard_maximum}")
        self.session = session or requests.Session()
        self.session.trust_env = False
        self.session.proxies.clear()
        self.session.headers.update(
            {
                "Accept": "application/json",
                "Accept-Encoding": "gzip, deflate",
                "User-Agent": "data-platform-football/espn-native-v2",
            }
        )
        self.sleep_fn = sleep_fn
        self.monotonic_fn = monotonic_fn
        self.utcnow_fn = utcnow_fn
        self._ledger: list[RequestLedgerEntry] = []
        self._lock = threading.RLock()

    @property
    def ledger(self) -> tuple[RequestLedgerEntry, ...]:
        with self._lock:
            return tuple(self._ledger)

    def flush(self) -> None:
        """Wait for queued raw writes; raises the first write failure."""
        self.raw_writer.flush()

    def close(self) -> None:
        self.raw_writer.close()

    def _append_ledger(self, entry: RequestLedgerEntry) -> RequestLedgerEntry:
        if entry.proxy_bytes != 0:
            raise AssertionError("ESPN proxy_bytes must always be zero")
        with self._lock:
            self._ledger.append(entry)
        return entry

    def _entry(
        self,
        target: CanonicalTarget,
        endpoint: EndpointType,
        *,
        started: float,
        disposition: str,
        attempts: Optional[_Attempts] = None,
        requested_at: Optional[str] = None,
        status: Optional[int] = None,
        record: Optional[RawJsonRecord] = None,
        error: Optional[str] = None,
        transport_origin: Optional[str] = None,
    ) -> RequestLedgerEntry:
        if attempts is not None:
            status = attempts.status
            transport_origin = attempts.origin
            requested_at = attempts.requested_at
        origin = transport_origin
        return self._append_ledger(
            RequestLedgerEntry(
                url_fingerprint=target.url_fingerprint,
                endpoint=endpoint,
                attempts=attempts.count if attempts else 0,
                status=status,
                direct_bytes=attempts.direct_bytes if attempts else 0,
                proxy_bytes=0,
                latency_ms=max(0.0, (self.monotonic_fn() - started) * 1000.0),
                raw_uri=record.raw_uri if record else None,
                content_hash=record.content_hash if record else None,
                disposition=disposition,
                error=error,
                transport_origin=origin,
                requested_at=requested_at,
                host=urlsplit(origin).hostname if origin else None,
                lane=self.lane,
                step=attempts.step if attempts else None,
                content_encoding=attempts.content_encoding if attempts else None,
                origin_attempts=tuple(attempts.origins) if attempts else (),
            )
        )

    def _now_iso(self) -> str:
        return self.utcnow_fn().astimezone(timezone.utc).isoformat()

    def _cached_result(
        self,
        target: CanonicalTarget,
        endpoint: EndpointType,
        body: bytes,
        record: RawJsonRecord,
        *,
        started: float,
        disposition: str = "cache_hit",
    ) -> FetchResult:
        # v1 aliases predate origin provenance, so their logical HTTPS origin
        # is also their actual transport origin.
        transport_origin = record.transport_origin or _transport_origin(
            target.canonical_url
        )
        try:
            data = json.loads(body)
        except (UnicodeDecodeError, json.JSONDecodeError) as exc:
            entry = self._entry(
                target,
                endpoint,
                started=started,
                requested_at=self._now_iso(),
                status=record.http_status,
                disposition="corrupt_cache",
                record=record,
                error="cached raw body is not valid JSON",
                transport_origin=transport_origin,
            )
            raise InvalidJsonError(
                "Cached ESPN raw body is not valid JSON", ledger_entry=entry
            ) from exc
        self._entry(
            target,
            endpoint,
            started=started,
            requested_at=self._now_iso(),
            status=record.http_status,
            disposition=disposition,
            record=record,
            transport_origin=transport_origin,
        )
        return FetchResult(
            target=target,
            endpoint=endpoint,
            json_data=data,
            body=body,
            attempts=0,
            status=record.http_status,
            cache_hit=True,
            direct_bytes=0,
            proxy_bytes=0,
            raw_uri=record.raw_uri,
            content_hash=record.content_hash,
            fetched_at=record.fetched_at,
            transport_origin=transport_origin,
        )

    def _load_cached(self, target: CanonicalTarget):
        pending = self.raw_writer.pending(target)
        if pending is not None:
            return pending
        return self.raw_store.load(target)

    def replay_json(
        self,
        url: str,
        endpoint: EndpointType | str,
        params: Optional[Params] = None,
    ) -> FetchResult:
        started = self.monotonic_fn()
        target = canonicalize_target(url, params)
        endpoint_type = EndpointType.parse(endpoint)
        body, record = self._load_cached(target)
        if record.endpoint != endpoint_type.value:
            raise RawTargetCorrupt(
                f"Raw ESPN endpoint mismatch: {record.endpoint} != {endpoint_type.value}"
            )
        return self._cached_result(target, endpoint_type, body, record, started=started)

    def fetch_json(
        self,
        url: str,
        endpoint: EndpointType | str,
        params: Optional[Params] = None,
        *,
        force_refresh: bool = False,
    ) -> FetchResult:
        started = self.monotonic_fn()
        target = canonicalize_target(url, params)
        endpoint_type = EndpointType.parse(endpoint)
        cluster = self.gate.policy.cluster_of(_transport_origin(target.canonical_url))

        if not force_refresh:
            try:
                body, record = self._load_cached(target)
                if record.endpoint != endpoint_type.value:
                    raise RawTargetCorrupt(
                        "raw endpoint metadata does not match request"
                    )
                return self._cached_result(
                    target, endpoint_type, body, record, started=started
                )
            except (RawTargetNotFound, RawTargetCorrupt):
                # A corrupt alias/blob is a cache miss; the content-addressed
                # rewrite replaces the damaged object.
                pass

        tries = _Attempts(self._now_iso())
        last_error = ""
        while tries.count < self.max_attempts:
            try:
                permit = self.gate.acquire(cluster)
            except (LaneClosed, AllOriginsBlocked, DailyCapExceeded) as exc:
                disposition = {
                    LaneClosed: "lane_closed",
                    AllOriginsBlocked: "blocked_deferred",
                    DailyCapExceeded: "daily_cap",
                }[type(exc)]
                exc.ledger_entry = self._entry(
                    target,
                    endpoint_type,
                    started=started,
                    attempts=tries,
                    disposition=disposition,
                    error=str(exc),
                )
                raise
            tries.start(permit)
            request_url = _on_origin(target.canonical_url, permit.origin)
            response = None
            body = b""
            try:
                response = self.session.get(
                    request_url,
                    timeout=(self.connect_timeout, self.read_timeout),
                    stream=True,
                    allow_redirects=False,
                )
                tries.finish(int(response.status_code))
                if 200 <= tries.status <= 299:
                    body, direct_bytes, encoding = self._read_response(response)
                    tries.direct_bytes += direct_bytes
                    tries.content_encoding = encoding
                    self.gate.report(
                        permit, status=tries.status, direct_bytes=direct_bytes
                    )
                    if encoding == "identity" and (
                        direct_bytes > self.gate.policy.uncompressed_warn_bytes
                    ):
                        logger.warning(
                            "ESPN answered %d bytes uncompressed from %s",
                            direct_bytes,
                            permit.origin,
                        )
                else:
                    outcome = self.gate.report(permit, status=tries.status)
            except _ResponseReadTimeout as exc:
                tries.direct_bytes += exc.direct_bytes
                tries.finish(None)
                self.gate.report(permit, timeout=True, direct_bytes=exc.direct_bytes)
                last_error = "timeout"
                if tries.count >= self.max_attempts:
                    break
                self.sleep_fn(self._retry_delay(tries.count, None))
                continue
            except (requests.Timeout, Urllib3ReadTimeoutError, TimeoutError):
                tries.finish(None)
                self.gate.report(permit, timeout=True)
                last_error = "timeout"
                if tries.count >= self.max_attempts:
                    break
                self.sleep_fn(self._retry_delay(tries.count, None))
                continue
            except _ResponseReadFailure as exc:
                tries.direct_bytes += exc.direct_bytes
                self.gate.report(
                    permit, status=tries.status, direct_bytes=exc.direct_bytes
                )
                tries.finish(None)
                self._raise_terminal(
                    DirectTransportError,
                    "Non-retryable ESPN transport error: " + exc.error_type,
                    f"non-timeout transport error: {exc.error_type}",
                    target,
                    endpoint_type,
                    started,
                    tries,
                    None,
                )
            except _ReadLimitExceeded as exc:
                tries.direct_bytes += exc.direct_bytes
                self.gate.report(
                    permit, status=tries.status, direct_bytes=exc.direct_bytes
                )
                self._raise_terminal(
                    ResponseTooLarge,
                    "response_too_large",
                    "response_too_large",
                    target,
                    endpoint_type,
                    started,
                    tries,
                    exc,
                    disposition="response_too_large",
                )
            except (_ResponseEncodingError, zlib.error) as exc:
                tries.direct_bytes += getattr(exc, "direct_bytes", 0)
                self.gate.report(
                    permit,
                    status=tries.status,
                    direct_bytes=getattr(exc, "direct_bytes", 0),
                )
                self._raise_terminal(
                    DirectTransportError,
                    "Invalid ESPN response encoding",
                    "invalid response encoding",
                    target,
                    endpoint_type,
                    started,
                    tries,
                    exc,
                )
            except (requests.RequestException, Urllib3HTTPError) as exc:
                tries.finish(None)
                self.gate.report(permit)
                self._raise_terminal(
                    DirectTransportError,
                    "Non-retryable ESPN transport error: " + type(exc).__name__,
                    f"non-timeout transport error: {type(exc).__name__}",
                    target,
                    endpoint_type,
                    started,
                    tries,
                    None,
                )
            finally:
                if response is not None:
                    response.close()

            status = tries.status
            if status == 403:
                outcome_error = (
                    AllOriginsBlocked if outcome.all_blocked else OriginBlocked
                )
                entry = self._entry(
                    target,
                    endpoint_type,
                    started=started,
                    attempts=tries,
                    disposition="blocked_deferred",
                    error=f"HTTP 403 on {permit.origin}",
                )
                raise outcome_error(
                    f"ESPN origin {permit.origin} answered 403; request deferred",
                    ledger_entry=entry,
                )

            if status in RETRYABLE_HTTP_STATUSES or 500 <= status <= 599:
                last_error = f"retryable HTTP {status}"
                if tries.count >= self.max_attempts:
                    break
                self.sleep_fn(
                    self._retry_delay(tries.count, response.headers.get("Retry-After"))
                )
                continue

            if not 200 <= status <= 299:
                entry = self._entry(
                    target,
                    endpoint_type,
                    started=started,
                    attempts=tries,
                    disposition="terminal_http_failure",
                    error=f"non-retryable HTTP {status}",
                )
                raise HttpStatusError(
                    status,
                    f"Non-retryable ESPN HTTP {status}",
                    ledger_entry=entry,
                )

            fetched_at = self._now_iso()
            try:
                record = self.raw_writer.put(
                    target,
                    endpoint_type,
                    body,
                    fetched_at=fetched_at,
                    http_status=status,
                    direct_bytes=tries.direct_bytes,
                    transport_origin=permit.origin,
                )
            except (RawStoreError, OSError, ValueError) as exc:
                entry = self._entry(
                    target,
                    endpoint_type,
                    started=started,
                    attempts=tries,
                    disposition="raw_store_failure",
                    error=f"raw store failure: {type(exc).__name__}",
                )
                raise DirectTransportError(
                    "ESPN raw response could not be committed", ledger_entry=entry
                ) from exc
            try:
                data = json.loads(body)
            except (UnicodeDecodeError, json.JSONDecodeError) as exc:
                entry = self._entry(
                    target,
                    endpoint_type,
                    started=started,
                    attempts=tries,
                    disposition="invalid_json",
                    record=record,
                    error="response is not valid JSON",
                )
                raise InvalidJsonError(
                    "ESPN response is not valid JSON", ledger_entry=entry
                ) from exc
            self._entry(
                target,
                endpoint_type,
                started=started,
                attempts=tries,
                disposition="success",
                record=record,
            )
            return FetchResult(
                target=target,
                endpoint=endpoint_type,
                json_data=data,
                body=body,
                attempts=tries.count,
                status=status,
                cache_hit=False,
                direct_bytes=tries.direct_bytes,
                proxy_bytes=0,
                raw_uri=record.raw_uri,
                content_hash=record.content_hash,
                fetched_at=record.fetched_at,
                transport_origin=record.transport_origin,
            )

        entry = self._entry(
            target,
            endpoint_type,
            started=started,
            attempts=tries,
            disposition="retry_exhausted",
            error=last_error,
        )
        raise RetryExhausted(
            f"ESPN retryable request failed after {tries.count} attempts: {last_error}",
            ledger_entry=entry,
        )

    def _raise_terminal(
        self,
        error_type,
        message: str,
        ledger_error: str,
        target: CanonicalTarget,
        endpoint: EndpointType,
        started: float,
        tries: _Attempts,
        cause: Optional[BaseException],
        *,
        disposition: str = "terminal_failure",
    ):
        entry = self._entry(
            target,
            endpoint,
            started=started,
            attempts=tries,
            disposition=disposition,
            error=ledger_error,
        )
        raise error_type(message, ledger_entry=entry) from cause

    def _read_response(self, response) -> tuple[bytes, int, str]:
        headers = response.headers or {}
        raw_limit = self.response_cap_bytes
        content_length = headers.get("Content-Length")
        declared = None
        if content_length is not None:
            try:
                declared = int(content_length)
            except (TypeError, ValueError):
                declared = None
            if declared is not None and declared > raw_limit:
                raise _ReadLimitExceeded(0)

        raw = response.raw
        if hasattr(raw, "decode_content"):
            raw.decode_content = False
        chunks = bytearray()
        while True:
            if declared is not None and len(chunks) == declared:
                break
            if len(chunks) >= raw_limit:
                raise _ReadLimitExceeded(len(chunks))
            try:
                chunk = raw.read(min(64 * 1024, raw_limit - len(chunks)))
            except (requests.Timeout, Urllib3ReadTimeoutError, TimeoutError) as exc:
                raise _ResponseReadTimeout(len(chunks)) from exc
            except (requests.RequestException, Urllib3HTTPError, OSError) as exc:
                raise _ResponseReadFailure(len(chunks), type(exc).__name__) from None
            if not chunk:
                break
            if len(chunk) > raw_limit - len(chunks):
                raise _ReadLimitExceeded(len(chunks))
            chunks.extend(chunk)
        encoded = bytes(chunks)
        encoding = str(headers.get("Content-Encoding", "")).lower().strip()
        try:
            if encoding in {"gzip", "x-gzip"}:
                body = self._bounded_decompress(encoded, 16 + zlib.MAX_WBITS)
            elif encoding == "deflate":
                try:
                    body = self._bounded_decompress(encoded, zlib.MAX_WBITS)
                except zlib.error:
                    body = self._bounded_decompress(encoded, -zlib.MAX_WBITS)
            elif not encoding or encoding == "identity":
                body = encoded
                encoding = "identity"
            else:
                raise ValueError(f"unsupported Content-Encoding {encoding!r}")
        except (ValueError, zlib.error) as exc:
            raise _ResponseEncodingError(len(encoded), exc) from exc
        if len(body) > self.response_cap_bytes:
            raise _ReadLimitExceeded(len(encoded))
        return body, len(encoded), encoding

    def _bounded_decompress(self, encoded: bytes, window_bits: int) -> bytes:
        decoder = zlib.decompressobj(window_bits)
        body = decoder.decompress(encoded, self.response_cap_bytes + 1)
        if len(body) > self.response_cap_bytes or decoder.unconsumed_tail:
            raise _ReadLimitExceeded(len(encoded))
        if not decoder.eof or decoder.unused_data:
            raise zlib.error("compressed payload is truncated or has trailing data")
        body += decoder.flush(self.response_cap_bytes + 1 - len(body))
        if len(body) > self.response_cap_bytes:
            raise _ReadLimitExceeded(len(encoded))
        return body

    def _retry_delay(self, attempts: int, retry_after: Optional[str]) -> float:
        if retry_after is not None:
            value = str(retry_after).strip()
            if re.fullmatch(r"(?:0|[1-9][0-9]*)(?:\.[0-9]+)?", value):
                delay = float(value)
                if math.isfinite(delay):
                    return min(60.0, delay)
            else:
                try:
                    retry_at = parsedate_to_datetime(value)
                    if retry_at.tzinfo is None:
                        retry_at = retry_at.replace(tzinfo=timezone.utc)
                    now = self.utcnow_fn()
                    if now.tzinfo is None:
                        now = now.replace(tzinfo=timezone.utc)
                    delay = (retry_at - now).total_seconds()
                    if math.isfinite(delay) and delay >= 0:
                        return min(60.0, delay)
                except (TypeError, ValueError, OverflowError):
                    pass
        return float(2 ** (attempts - 1))


__all__ = [
    "AllOriginsBlocked",
    "AmbientProxyError",
    "CanonicalTarget",
    "DailyCapExceeded",
    "DEFAULT_CONNECT_TIMEOUT",
    "DEFAULT_MAX_ATTEMPTS",
    "DEFAULT_READ_TIMEOUT",
    "DEFAULT_RESPONSE_CAP_BYTES",
    "DirectTransportError",
    "EndpointType",
    "EspnHttpClient",
    "EspnTransportError",
    "FetchResult",
    "HttpStatusError",
    "InvalidJsonError",
    "LaneClosed",
    "OriginBlocked",
    "RequestLedgerEntry",
    "ResponseTooLarge",
    "RetryExhausted",
    "TransportGate",
    "canonicalize_target",
]
