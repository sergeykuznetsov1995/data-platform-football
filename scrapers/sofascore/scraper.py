...
def _season_year_label(league: str, season) -> str:
    """SofaScore API year for the exact competition-season.

    Split-year leagues use ``25/26`` while single-year tournaments keep the
    literal ``2026``. Keeping this next to ``_season_label`` prevents shared
    capture paths from silently converting World Cup 2026 to ``20/26``.
    """
    from scrapers.sofascore.camoufox_capture import season_short_to_label

    label = _season_label(league, season)
    if _is_single_year(league, season):
        return label
    return season_short_to_label(label)
...
class SofaScoreScraper(SoccerdataScraper):
    ...
    def read_schedule(self) -> Optional[pd.DataFrame]:
        ...
        for league in self.leagues:
            ...
            if ut_id is None or slug is None:
                ...
                continue
            season_slug = _season_label(league, self.seasons[0])
            target_year = _season_year_label(league, self.seasons[0])
    ...
    def read_league_table(self) -> Optional[pd.DataFrame]:
        ...
        for league in self.leagues:
            ...
            if ut_id is None or slug is None:
                ...
                continue
            season_slug = _season_label(league, self.seasons[0])
            target_y = _season_year_label(league, self.seasons[0])
            nav_url = f"https://www.sofascore.com/tournament/{slug}/{ut_id}"
    ...
    def read_tournament_snapshot(
        ...
        for league in self.leagues:
            ...
            if ut_id is None or slug is None:
                ...
                continue
            season_short = _season_label(league, self.seasons[0])
            target_year = _season_year_label(league, self.seasons[0])
            nav_url = f"https://www.sofascore.com/tournament/{slug}/{ut_id}"
    ...
    def read_player_capture(
        ...
        target_season_label = _season_year_label(league, season)
