...
class NodriverFBrefScraper(BaseScraper):
    ...
    def __init__(
        ...
        # Issue #616: per-URL byte/request breakdown, flushed across restarts.
        self._real_traffic_base_bytes_by_url: Counter = Counter()
        self._real_traffic_base_requests_by_url: Counter = Counter()
    ...
    def _update_real_traffic_stats(self) -> None:
        ...
        for k, v in (session.get('real_bytes_by_url') or {}).items():
            bytes_by_url[k] = bytes_by_url.get(k, 0) + v
        for k, v in (session.get('real_requests_by_url') or {}).items():
