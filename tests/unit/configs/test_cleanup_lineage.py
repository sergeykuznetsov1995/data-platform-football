...
* the curated list targets only ``gold`` tables and excludes
  ``entity_xref`` (not part of epic #478; its own drop is the followup #211),
...
def test_curated_list_shape() -> None:
    """19 #478 drops, all gold FQNs, entity_xref excluded (#211)."""
    mod = _import_cleanup()
    ...
    assert not any("entity_xref" in f for f in mod.CURATED_FQNS), "entity_xref is not part of #478 (its drop is #211), must not be targeted"
