...
def test_scheduled_discovery_poison_proxy_and_bot_pr_contract():
    ...
    assert (
        ...
        "format('sofascore-registry-discovery-pr-{0}', "
        "github.event.pull_request.number) || 'sofascore-registry-discovery' }}"
        in workflow
    )
    assert "sofascore-registry-discovery-${{" not in workflow
    assert "|| 'trusted'" not in workflow
