"""Capology residential-proxy byte counter (issue #789 Phase 2).

Mirrors the Transfermarkt counter: the scraper passively counts response-body
bytes per host so the daily proxy report can attribute residential spend to
Capology. The counter must count every status (a 403 CF page is billed too),
aggregate by host, and never raise on a malformed response.
"""
...
@pytest.fixture
def scraper():
    return CapologyScraper(leagues=['ENG-Premier League'], seasons=[2025])


@pytest.mark.unit
def test_record_proxy_bytes_accumulates_by_host(scraper):
    scraper._record_proxy_bytes('https://www.capology.com/a', _FakeResp(b'a' * 1000))
    scraper._record_proxy_bytes('https://www.capology.com/b', _FakeResp(b'b' * 2000))
...
def test_counts_error_pages_too(scraper):
    ...
    scraper._record_proxy_bytes(
        'https://www.capology.com/z', _FakeResp(b'x' * 500, status=403)
    )
