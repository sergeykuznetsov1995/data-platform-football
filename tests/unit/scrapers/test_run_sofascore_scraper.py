...
def _season_label(league: str, season) -> str:
    ...
    Keep the configured single-year tournament literal and use the club
...
class TestSeasonTokenNoShift:
    ...
    @pytest.mark.unit
    def test_single_year_tournament_keeps_literal_partition(self, temp_output):
        with patch("utils.medallion_config.get_active_season", return_value=2026):
            rc, seen = self._record_resolve_season(
                [
                    "--entity",
                    "match_capture",
