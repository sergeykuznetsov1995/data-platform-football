...
@pytest.mark.unit
def test_master_waits_for_exact_scheduled_understat_run_fail_closed():
    calls = _operator_calls("dag_master_pipeline.py", "ExternalTaskSensor")
    sensor = next(
        call
        for call in calls
        if _literal(call["external_dag_id"]) == "dag_ingest_understat"
    )
    assert _literal(sensor["task_id"]) == "wait_for_scheduled_understat"
    assert _literal(sensor["allowed_states"]) == ["success"]
    assert _literal(sensor["failed_states"]) == ["failed"]
    assert _literal(sensor["mode"]) == "reschedule"
    assert _literal(sensor["check_existence"]) is True
...
def test_master_senses_scheduled_fbref_then_runs_direct_gold_prerequisites():
    ...
    assert "ingestion_triggers.trigger_understat" not in tasks
    understat_sensor = tasks["wait_for_scheduled_understat"]
    ...
    assert understat_sensor.task_id in tasks["validate_required_sources"].upstream_task_ids
