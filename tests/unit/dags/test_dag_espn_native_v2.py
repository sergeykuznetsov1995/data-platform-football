...
def test_discovery_policy_rejects_legacy_site_catalog_shape():
    from dags.utils import espn_native_tasks
    ...
    with pytest.raises(espn_native_tasks.OperationsError, match="Core catalog"):
        espn_native_tasks._normalize_discovery_dropdown(site_catalog)
...
def test_discovery_catalog_rejects_non_core_runtime_override(monkeypatch):
    from dags.utils import espn_native_tasks
    ...
    monkeypatch.setattr(
        espn_native_tasks.PostgresEspnControlStore,
        "from_env",
        classmethod(lambda _cls: pytest.fail("invalid URL must fail before DB access")),
    )
    monkeypatch.setattr(
        espn_native_tasks,
        "_http_client",
        lambda *_a, **_k: pytest.fail("invalid URL must fail before network access"),
    )

    with pytest.raises(
        espn_native_tasks.OperationsError, match="authoritative Core catalog"
    ):
        espn_native_tasks.fetch_discovery_catalog()
...
@pytest.mark.parametrize(
    ("publication_state", "expected_outcome"),
    [("complete", "complete_new"), ("noop", "noop_revalidated")],
)
def test_scope_qualification_records_dispositions_and_fresh_revalidation(
    monkeypatch, publication_state, expected_outcome
):
    from dags.utils import espn_native_tasks
    from scrapers.espn.models import DispositionState, RequestDisposition

    final = SimpleNamespace(
        event_id=11, played_final=True, summary_required=True
    )
    nonfinal = SimpleNamespace(
        event_id=12, played_final=False, summary_required=False
    )
    generation = SimpleNamespace(
        ...
        raw_ledger=(
            SimpleNamespace(
                ...
                raw_uri="s3://raw/scoreboard.json",
                raw_sha256="e" * 64,
            ),
        ),
        dispositions=(
            RequestDisposition("lineup", "captured", "both", 11),
            RequestDisposition("matchsheet", "valid_empty", "empty", 11),
            RequestDisposition("lineup", "not_applicable", "nonfinal", 12),
            RequestDisposition("matchsheet", "not_applicable", "nonfinal", 12),
        ),
    )
    monkeypatch.setattr(
        espn_native_tasks,
    ...
    evidence_ref = {"uri": "s3://evidence/run.json", "sha256": "f" * 64}

    result = espn_native_tasks._scope_qualification_payload(
        publication={"state": publication_state, "evidence_ref": evidence_ref},
        generation=generation,
        evidence_payload={"recorded_at": "2026-08-08T12:00:00+00:00"},
    )

    assert result["outcome"] == expected_outcome
    assert result["schedule"] == {"state": "captured", "failures": []}
    assert result["events"] == [
        {
            ...
            "entities": {
                "lineup": {"state": "captured", "failures": []},
                "matchsheet": {"state": "valid_empty", "failures": []},
            },
            "failures": [],
        },
        {
            ...
            "entities": {
                "lineup": {"state": "not_applicable", "failures": []},
                "matchsheet": {"state": "not_applicable", "failures": []},
            },
            "failures": [],
        },
    ]
    assert result["raw_evidence"][0]["raw_uri"] == "s3://raw/scoreboard.json"
    assert result["raw_evidence"][0]["raw_sha256"] == "e" * 64
...
def test_scope_qualification_rejects_mixed_parser_runtime(monkeypatch):
    from dags.utils import espn_native_tasks
    ...
    with pytest.raises(espn_native_tasks.OperationsError, match="parser-v3/runtime-v4"):
...
def test_release_qualification_identity_binds_exact_target_and_versions(monkeypatch):
    from dags.utils import espn_native_tasks
    ...
    index = {
        "registry_signature": "c" * 64,
        "scope_ids": ["2:2026", "1:2026"],
    }

    result = espn_native_tasks._release_qualification_identity(index)

    assert result["release_commit"] == "a" * 40
    assert result["release_tree_sha256"] == "b" * 64
    assert result["registry_signature"] == "c" * 64
    assert result["target_scope_ids"] == ["1:2026", "2:2026"]
    assert len(result["target_scope_sha256"]) == 64
    assert result["parser_version"] == "espn-native-parser-v3"
...
def test_saved_detail_evidence_reaches_male_registry_artifact(
    ...
    competition = {"espn_id": None, "slug": "ita.1", "name": "ita.1"}
    batch = {
        "kind": "espn-discovery-detail-batch-v1",
        "batch_id": "batch-1",
    ...
    dropdown = {
        ...
        "items": [
            {
                "$ref": "https://sports.core.api.espn.com/v2/sports/soccer/leagues/ita.1?lang=en&region=us"
            }
        ],
    }
    saved = {
    ...
    assert result["discovery_state_ref"]["uri"].endswith("/discovery-state.json")
    checkpoint = next(
        payload
        for payload in written
        if payload.get("kind") == "espn-discovery-reducer-checkpoint-v2"
    )
    state_uri, state_payload, state_kwargs = next(
        write for write in writes if write[0].endswith("/discovery-state.json")
    )
    latest_uri, latest_payload, latest_kwargs = next(
        write for write in writes if write[0].endswith("/discovery/latest-state.json")
    )
    assert checkpoint["discovery_state_payload"] == state_payload
...
def test_discovery_reducer_rejects_partial_index_before_phase_reduction(monkeypatch):
    from dags.utils import espn_native_tasks

    raw_ref = {"uri": "s3://artifacts/raw-ref.json", "sha256": "a" * 64}
    index_ref = {"uri": "s3://artifacts/index-ref.json", "sha256": "b" * 64}
    batch_ref = {"uri": "s3://artifacts/batch-ref.json", "sha256": "c" * 64}
    phase_ref = {"uri": "s3://artifacts/phase-ref.json", "sha256": "d" * 64}
    competition = {"espn_id": None, "slug": "eng.1", "name": "eng.1"}
    raw = {
        "kind": "espn-discovery-raw-v1",
        "captured_at": "2026-08-01T00:00:00+00:00",
        "raw_uri": "s3://raw/core-catalog.json",
        "raw_sha256": "e" * 64,
        "proxy_bytes": 0,
    }
    index = {
        "kind": "espn-discovery-detail-index-v1",
        "discovery_raw_ref": raw_ref,
        "captured_at": raw["captured_at"],
        "batch_ids": ["batch-1"],
        "batch_refs": [batch_ref],
        "competition_count": 1,
    ...
    monkeypatch.setattr(
        espn_native_tasks,
        "_read_ref",
        lambda ref, **_kwargs: payloads[ref["uri"]],
    )
    monkeypatch.setattr(
        espn_native_tasks.EspnRawStore,
        "from_uri",
        lambda _uri: SimpleNamespace(
            load_exact=lambda uri, _sha: (
                json.dumps(core_catalog).encode()
                if uri == raw["raw_uri"]
                else pytest.fail("partial index must fail before detail evidence reads")
            )
        ),
    )
    monkeypatch.setattr(
        ...
        lambda *_a, **_k: pytest.fail(
            "partial index must fail before publishing reducer artifacts"
        ),
    )
...
def test_discovery_review_retry_replays_checkpoint_without_recomputation(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda uri, payload, **kwargs: (
            writes.append((uri, payload, kwargs))
            or espn_native_tasks._payload_ref(uri, payload)
        ),
    )
    monkeypatch.setattr(
        espn_native_tasks.PostgresEspnControlStore,
    ...
    assert result["male_registry_ref"] == male_registry_ref
    assert result["discovery_state_ref"]["uri"].endswith("/discovery-state.json")
    assert len(writes) == 2
    state_uri, immutable_state, state_kwargs = writes[0]
    latest_uri, latest_state, kwargs = writes[1]
    assert state_uri.endswith("/discovery-state.json")
    assert latest_uri.endswith("/discovery/latest-state.json")
    assert immutable_state == latest_state
...
def test_discovery_partial_publication_replays_sealed_reducer_checkpoint(monkeypatch):
    ...
    state_uri = f"{root}/discovery-state.json"
    ...
    sealed_v2 = {
        **checkpoint,
        ...
        "schema_version": 2,
        "discovery_state_payload": state,
        "discovery_state_ref": state_ref,
    }
    tampered_v2 = json.loads(json.dumps(sealed_v2))
    tampered_v2["discovery_state_payload"]["male_scope_count"] = 2
    with pytest.raises(
        espn_native_tasks.OperationsError, match="state binding mismatch"
    ):
        espn_native_tasks._validate_discovery_reducer_checkpoint(
            tampered_v2,
            discovery_detail_index_ref=index_ref,
            discovery_detail_phase_refs=[phase_ref],
    ...
    assert writes[-2][1] == state
    assert writes[-1][1] == state
    assert writes[-2][2] == {}
    assert writes[-1][2] == {"immutable": False}


def test_older_immutable_discovery_state_loads_after_latest_alias_advances(
    monkeypatch, tmp_path
):
    from dags.utils import espn_native_tasks
    ...
    def publish(run_name: str, observed_at: str):
        ...
        candidate_ref = espn_native_tasks._write_payload(
            espn_native_tasks._join_uri(run_root, "candidate.json"), candidate_payload
        )
        registry_ref = espn_native_tasks._write_payload(
            espn_native_tasks._join_uri(run_root, "male-registry.json"),
            registry_payload,
        )
        review = {
            "kind": "espn-discovery-review-v2",
    ...
    loaded_registry, discovery = espn_native_tasks._load_discovered_registry(
        now=datetime(2026, 8, 20, tzinfo=timezone.utc)
    )

    latest_ref = espn_native_tasks._ref_for_uri(
        espn_native_tasks._join_uri(artifact_root, "discovery", "latest-state.json")
    )
    assert espn_native_tasks._read_ref(first_ref) == first_state
    assert espn_native_tasks._read_ref(latest_ref) == second_state
    assert discovery["discovery_state_ref"] == first_ref
    assert loaded_registry.signature() == registry.signature()
...
def test_published_dq_verifies_current_views_for_the_active_route(
    ...
    evidence = {"recorded_at": completed_at.isoformat(), "lease_epoch": 7}
    descriptor = {
        "dag_id": head.dag_id,
        "scope_root": "s3://artifacts/scope",
    }
...
def test_monitor_retains_physical_head_freshness_from_another_registry(monkeypatch):
    from dags.utils import espn_native_tasks
    ...
    assert observed[0]["state"] == "complete"
    assert observed[0]["last_complete_at"] == database_now
...
def test_final_leaf_seals_success_only_after_health_and_release(monkeypatch):
    ...
    payloads["durable-run-manifest.json"].update(
        release=espn_native_tasks._release_qualification_identity(
            payloads["plan-index.json"]
        ),
    ...
    assert written["payload"]["release"] == payloads["durable-run-manifest.json"][
        "release"
    ]
...
def test_exact_v4_noop_attestation_refreshes_without_moving_old_head(monkeypatch):
    ...
    old = datetime(2026, 7, 29, 9, tzinfo=timezone.utc)
    fresh = datetime(2026, 8, 1, 14, 30, tzinfo=timezone.utc)
    head = ScopeHead(
        dag_id="dag_ingest_espn",
        scope_id="700:2026",
        generation_id="physical-generation",
        generation_signature="a" * 64,
        manifest_sha256="b" * 64,
        snapshot_uri="s3://artifacts/physical/generation.json",
        snapshot_sha256="c" * 64,
        registry_signature="d" * 64,
        plan_signature="e" * 64,
        run_id="physical-run",
    ...
    evidence = RunManifestEvidence(
        ...
        registry_signature=head.registry_signature,
        state="noop",
        evidence_uri="s3://artifacts/qualification/run-evidence.json",
        evidence_sha256="1" * 64,
        recorded_at=fresh,
    )
    selected = espn_native_tasks._head_to_dict(head)
    evidence_ref = {"uri": evidence.evidence_uri, "sha256": evidence.evidence_sha256}
    publication_ref = {
        "uri": "s3://artifacts/qualification/publication.json",
        "sha256": "2" * 64,
    }
    dq_ref = {
        "uri": "s3://artifacts/qualification/published-dq.json",
        "sha256": "3" * 64,
    }
    ...
    publication = {
        ...
        "selected_head": selected,
        "publication_intent_ref": None,
        "scope_binding_ref": attestation["scope_binding_ref"],
    }
    dq = {
        "publication_ref": publication_ref,
        "dag_id": evidence.dag_id,
    ...
    def read_ref(_ref, *, kind=None):
        return {
            "espn-run-manifest-evidence-v1": evidence_payload,
            "espn-scope-qualification-attestation-v1": attestation,
            "espn-publication-result-v1": publication,
            "espn-published-dq-result-v1": dq,
        }[kind]
    ...
    monkeypatch.setattr(
        espn_native_tasks,
        "_verified_complete_head",
        lambda *_args, **_kwargs: (head, "complete"),
    )
    monkeypatch.setattr(espn_native_tasks, "_read_ref", read_ref)
    monkeypatch.setattr(
        espn_native_tasks,
        ...
        lambda _ref: (
            ...
            SimpleNamespace(
                ...
                bindings={
                    evidence.scope_id: SimpleNamespace(
                        prior=SimpleNamespace(
                            uri=head.snapshot_uri,
                            artifact_sha256=head.snapshot_sha256,
                            scope_id=head.scope_id,
    ...
    mismatched_evidence = replace(evidence, registry_signature="0" * 64)
    assert (
        espn_native_tasks._qualified_freshness_at(
            head,
            mismatched_evidence,
...
@pytest.mark.parametrize("state", ["complete", "noop"])
def test_qualification_state_accepts_only_terminal_success_states(state):
    from dags.utils import espn_native_tasks

    assert espn_native_tasks._qualification_state(state) == state


@pytest.mark.parametrize("state", ["staged", "failed", "skipped", None, True])
def test_qualification_state_rejects_every_other_value(state):
    from dags.utils import espn_native_tasks
    from scrapers.espn.operations import OperationsError
