...
@pytest.mark.unit
def test_non_opta_scope_chunks_every_candidate_without_a_probe_stage(
    monkeypatch, tmp_path
):
    # #1474: the 2-match probe (matchIsOpta gate, PROBE_SAMPLE_SIZE) is gone;
    # stage availability lives in the repository candidate policy.
    state = _state(monkeypatch, tmp_path)
    ...
    schedule = state.pending_work("no-probe", plan["plan_id"])[0]
    state.append_receipt(
        queue_id="no-probe",
        plan_id=plan["plan_id"],
        work_item=schedule,
        outcome=_schedule_outcome(
            ...
            preview_game_ids=candidates,
        ),
    )

    chunks = state.pending_work("no-probe", plan["plan_id"])

    assert [item["kind"] for item in chunks] == ["matches", "matches", "matches"]
    assert [value for item in chunks for value in item["game_ids"]] == candidates
    with pytest.raises(WhoScoredOpsStoreError, match="unknown work item kind"):
        WhoScoredBackfillState.validate_work_item(
            plan,
            {
                "work_id": "probe-x",
                "kind": "probe",
                "scope": "WS-252-26=2526",
                "game_ids": [1, 52],
                "candidate_game_ids": candidates,
            },
        )
