...
## 5d. Runbook: деградация FlareSolverr («tab crashed», 2026-07-02)

Симптом: каждый `request.get` на тяжёлую страницу (WhoScored /Matches/*/Live)
мгновенно (<1.5 с) падает с `Error solving the challenge. Message: tab crashed`,
при этом лёгкие страницы (example.com) проходят. Причина: контейнер
`flaresolverr` ограничен **512 МБ**; после нескольких часов тяжёлого скрейпа
(или при параллельной чужой сессии — SoFIFA+WhoScored конкурируют за один
контейнер) baseline Chrome вырастает до ~300–400 МиБ, и новая тяжёлая вкладка
(~200–300 МБ) не помещается → renderer убивается.

Диагностика:
1. `docker stats --no-stream flaresolverr` — MEM% > ~60% при простое = деградация.
2. `curl -s -X POST http://localhost:8191/v1 -d '{"cmd":"sessions.list"}'` —
   чужая живая сессия (`sofifa-*`, `whoscored-*`) означает, что кто-то работает:
   НЕ рестартовать, дождаться/согласовать.
3. Счётчик `cf_challenge_failures` в ключе `traffic` output-JSON раннеров растёт.

Лечение: `docker restart flaresolverr` (сессии теряются — только когда нет
активных потребителей). Профилактика: поднять `mem_limit` flaresolverr в
compose.yaml до 2G — иначе длинный backfill (#715: сотни матчей подряд)
деградирует контейнер за ~3 часа; вчерашний прогон 1617/1718/1920 умер именно
так (151/251/50 из 380).
