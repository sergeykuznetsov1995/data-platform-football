...
Оркестратор `*/5 * * * *`. Фоновая полоса стартует до **11:45 UTC**
(`BACKGROUND_HOLD_START`), кооперативный дедлайн **13:15** (`BACKGROUND_DEADLINE` =
`DAILY_WINDOW_START` − `BACKGROUND_TAIL_ALLOWANCE` 45 мин: наблюдаемый хвост доскрапа и
Silver — 18–38 мин). Разбег фона не меньше `MIN_BACKGROUND_RUNWAY` 90 мин — волна-огрызок
не рождается. Дневная полоса стартует в окне **14:00–19:00 UTC** (`DAILY_WINDOW_END`),
кооперативный дедлайн **21:00**. После 19:00 `choose_lane` отдаёт
`background_window_closed` и до полуночи ничего не запускает: волна, рождённая в 19:45+,
держала бы контур занятым в окне ночной доставки (19:45–23:20 UTC).

Две суточные гарантии (#1282):

* **refresh каждые сутки.** Если за текущие UTC-сутки нет успешной волны `mode=refresh`,
  фоновая полоса = REFRESH независимо от курсора (`refresh_daily_guarantee`); после успеха
  курсор уходит на BACKFILL и остаток дня достаётся истории. Факт берётся одним запросом к
  метабазе (`_ingest_child_observations`), а не из состояния планировщика: форма Variable
  `fotmob.scheduler.state.v1` — четыре ключа — контракт отката пином.
* **пауза после красной волны.** `FAILURE_BACKOFF` 30 мин для любой полосы; до #1282
  следующая волна стартовала через 17–70 с и повторяла тот же отказ. Красная дневная полоса
  теперь повторяется в те же сутки — после паузы, пока окно старта открыто.

Размен, принятый осознанно: полоса BACKFILL контура почти не будет стартовать (история
живёт кампанией `/root/fotmob_history_backfill`), поэтому `lane_idle_h` в метриках волн
будет постоянно большим — это не авария. Поздний daily успевает меньше каталога, чем
ранний: потолок `_LANE_CAPS` для него декоративен, останов даёт `DAILY_DEADLINE`.

Окна DAG кампанию истории не задевают: `driver.sh` ходит мимо DAG (`docker exec` скрапера) и
при `WRITER_LOCK_SCOPE = "flush"` работает круглосуточно. Но комментарий
`driver.sh:100-112` обосновывает `WINDOW_HEAD_GUARD_MIN=905` старым `DAILY_WINDOW_END`
15:00 — при возврате широкого замка писателя это число пересчитать (чекбокс в #1282).
