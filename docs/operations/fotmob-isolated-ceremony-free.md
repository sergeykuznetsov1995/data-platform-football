...
- **`plan_signature` контентная** — журнал прогресса не переиспользуется между ранами с
  разным составом каталога. Отсюда: полоса истории каждый раз начинает сначала, правило
  «дыру источника подтверждаем со второй попытки» никогда не доходит до второй попытки,
  а порядок обхода каждый раз стартует с головы очереди.
- **Курсор полос теперь двигается, а полоса истории прогресс не копит.** После зелёного
  `refresh` следующей фоновой полосой становится `backfill` — до починки подписи он
  перечитывает одну и ту же голову исторической очереди. Рычаг: Variable
  `fotmob.scheduler.state.v1` в метабазе изолята, поле `next_background_lane`.
- **Терминальный исход стал тихим.** Скоуп, у которого сорвался коммит завершения,
  получает `outcome="terminal"`, краснит ТОТ ран — и дальше молча выпадает из всех планов
  (`planner.py` пропускает такой скоуп безусловно), потому что приёмка непрерывной полосы
  про историю больше не высказывается. Нужен TTL терминального состояния и счётчик в
  стороже; едет вместе с починкой подписи.
