...
- Раны оркестратора **красные штатно**: гейт статуса считает попытки по всей истории
  подписи, а не по текущему рану. Бронза при этом пишется по ходу рана (коммит на каждую
  цель идёт до гейта), поэтому сбор работает.
- Следствие: `advance_fotmob_scheduler_state` не выполняется → курсор полос стоит на
  `refresh` → **полоса истории не запускается**.
- Следствие: `trigger_silver_transform` не выполняется → **Silver надо триггерить руками**
  (`dag_transform_fotmob_silver`, `schedule=None`, не на паузе).
- `plan_signature` стал контентным → журнал прогресса бэкфила не переиспользуется между
  ранами с разным составом каталога.
