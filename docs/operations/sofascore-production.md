...
- Ретенция: при каждом сбросе и `finish` из книги уходят раны без активного claim старше
  ...
  в логе шлюза («allocation ledger compacted»). Замер на копии книги истории 23.09:
  1 480 → 300 ранов, 29,0 → 2,4 МБ, запись 18,8 мс (медиана).

**Восстановление после падения.** При старте шлюз доигрывает незакрытые аренды из WAL.
Если WAL насчитал по аренде больше байт, чем несброшенная книга, разница доигрывается в
книгу с предупреждением «journal ahead of ledger by N bytes, replayed» (раньше это был
фатальный `RuntimeError` и шлюз не стартовал); затем книга сразу сбрасывается на диск.
Байты эндпоинта, который был открыт в момент падения, в WAL не попадают (эндпоинт пишется
в WAL при закрытии) — книга аллокации теряет их за последние ≤ 10 с. Суточный бюджет это не
задевает: он считается по `paid_requests.jsonl` (добавление + `fsync` на чанк, #1056).
