...
- «Поздно размечено» — доказанно: прошлый observe (`prev_observe_at`, `state/sofascore_veha1/observe.json`) был
  позже старт + 24 ч и матча `finished` не видел. В критерий не входит, считается отдельно. Без прошлого observe
  (первый прогон) или при раннем матч остаётся в знаменателе.
...
ещё не отчитанные (плюс записи с `reported_day == D` — след оборванного прогона). Итог — одна строка сводки
...
Запись: нарушители → журнал → `daily.jsonl` (точка фиксации). Повторный запуск за тот же D печатает
сохранённую строку; `--dry-run` ничего не создаёт и замок не берёт. Сбой — строка
`⚠️ измеритель вехи 1: <причина>`, сводка не падает.

**Ограничения.** Первое наблюдение — раз в сутки (observe только из сводки): «поздно размечено» доказывается
лишь для матчей, которые прошлый прогон не видел `finished` позже 24 ч после старта; остальные остаются в
знаменателе. Закрытие, перезаписанное MERGE до первого запроса измерителя (в т. ч. весь хвост первого прогона),
видно только по новому времени. Окончание (`end_ts`) фиксируется при первом наблюдении. До доставки релиза с
