...
2. `bash deploy/sofascore/deploy.sh <дерево> [старое-дерево]` — вне окна 13:55–15:35 UTC:
   ...
   **Припаркованный повтор ломается принудительно.** Упавший скоуп уходит в `up_for_retry`,
   ...
   оплаченный скоуп доехал до `state.json` (capture: `finalize` успешен и `validate`
   терминален; metadata: скоуп терминален). Файл ни на что не влияет и код возврата не
   меняет — его читает автомат ночной доставки и кладёт в отчёт окна. Старый файл удаляется
   в начале шага, новый пишется атомарно.
