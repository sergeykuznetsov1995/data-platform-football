...
2. `bash deploy/sofascore/deploy.sh <дерево> [старое-дерево]` — вне окна 13:55–15:35 UTC:
   ...
   висит `running` навсегда. Но и **завершения** того прогона, который держал слот пула на
   входе, ждать нельзя: у `run_historical_scope` есть `retries=1` (#1244), и скоуп, упавший
   уже ПОСЛЕ осушения, встаёт за слотом, который этот же шаг только что забрал, — прогон
   висит `running` без единой движущейся задачи (ночь 05.09: ответ `0|0|1|0` все 5515 с,
   код 4, доставки не было). Ждём **движимую работу** того прогона (его run_id читается
   сразу после осушения; метабаза не ответила — код 4, выкат не начинаем): задачи в
   `queued`/`running` и задачи, стоящие за слотом НЕ осушённого пула —
   `validate_historical_scope` и `finalize_historical_run` в `default_pool`, они и
   засчитывают оплаченный скоуп. Припаркованный повтор в `sofascore_history_pool` движимой
   работой не считается (двинуться он не может по построению, а оплаченного результата за
   ним нет: первая попытка упала), охлаждение `wait_before_next_continuous_run` — тоже.
   Плюс отсутствие задач истории, дейли и актуалки и закрытых прогонов дейли.
