...
пересекают окно ночной доставки 03:30–06:00: `deploy.sh` пересоздаёт
...
2. `bash deploy/sofascore/deploy.sh <дерево> [старое-дерево]` — вне окна 13:55–15:35 UTC:
   ...
   **Припаркованный повтор ломается принудительно.** Упавший скоуп уходит в `up_for_retry`,
   ...
   (`docker exec -i … python -`): под блокировкой строки `dag_run` он переводит
   единственный mapped-скоуп из `up_for_retry` (или `scheduled` с `try_number > 1`) в
   `failed`, дальше планировщик сам доводит прогон до конца — `validate` получает
   ...
   припаркованные скоупы`; выход — только `0|0|0|0|0`. Второе число считает задачи дейли,
   актуалки, **полосы профилей игроков** и обслуживания манифеста: оплаченный скоуп профилей
   нельзя оборвать пересозданием scheduler'а, а паузой полосу закрыть нельзя (под паузой не
   отработает `validate_players_scope`) — её дверь закрывает пул
   `sofascore_players_pool`. Пятое число видно и когда ломатель отказал: скоуп в чужом
   пуле или в чужом прогоне даёт `…|1` и честный код 4 вместо тихого зависания.
