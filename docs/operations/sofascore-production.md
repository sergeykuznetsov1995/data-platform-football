...
токена — это **диагностика застоя цикла событий**, а не замер авторизованного обработчика
(`_control_report` и сериализация ответа 200 в него не входят). Приёмка #1350: p95 `/health`
и p95 `stats`(401) < 50 мс при идущем батче; `ReadTimeout` служебного канала в логах задач —
0/сутки. Прямой p95 авторизованного `stats` (200) этим пробником не снимается: нужен
токен аренды — см. follow-up в #1350.
