...
- Ретенция: при каждом сбросе и `finish` раны без активного claim старше 3 суток
  ...
  аллокацию. Облегчённый ран, не менявшийся 30 суток (`ALLOCATION_RUN_RETIRE_SECONDS`),
  ...
  копии книги истории 23.09: 1 480 ранов, 29,0 → 4,2 МБ, запись 30,7–38,1 мс (медиана),
  максимум 48,1 мс.
...
заведёт с нулевым расходом. Штатно это не случается: автомат ночной доставки откатывает
на предыдущий релиз в ту же ночь, а удаление начинается только после 30 суток работы
нового кода (сначала облегчение, удаление — через 30 суток без изменений), так что на
момент любого штатного отката удалённых ранов нет. DagRun старше 30 суток давно закрыт
(`dagrun_timeout` истории 6 ч, актуалки 7 ч; у дейли `dag_ingest_sofascore` таймаута
нет, но его прогоны суточные) — предъявить план он может только при ручном `clear`.
**Принятое ограничение:** ручной `clear` DagRun старше 30 суток на старом бинаре после
отката выдаст этому рану бюджет заново. Вечное хранение облегчённых ранов отвергнуто:
≈0,08 МБ/сут у истории возвращает рост книги и цены записи — тот же дефект через год.
