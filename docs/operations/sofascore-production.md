...
`environment:` — DAG его не увидит, сколько ни правь хостовый файл. Ровно так сейчас с
`SOFASCORE_HISTORY_RATE_LIMIT_PER_MINUTE` (`dag_backfill_sofascore_all_mens.py:69`): DAG
его читает, в рецепте его нет, и кампания идёт на дефолтном темпе. Отдельная ручка — per-source предел
