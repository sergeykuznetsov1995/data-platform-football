...
и не провал: автомат ждёт в цикле до 480 с.
...
через `sofascore_set_env_var` и **обязательная сверка перечитыванием** → оба compose из
старого дерева → рестарт трёх сторожей → пулы и паузы из снимка → подтверждение теми же
шестью признаками против старого дерева.
