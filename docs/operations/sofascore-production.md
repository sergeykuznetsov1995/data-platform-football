...
- «Поздно размечено» — источником, по двум уликам: `changes_change_timestamp` записи (последняя правка у SofaScore)
  позже старт + 24 ч **и** прошлый observe (`prev_observe_at`, `state/sofascore_veha1/observe.json`) был позже
  старт + 24 ч и матча `finished` не видел. Одна наша улика доказывала бы лишь отставание сбора афиши. В критерий
  не входит, считается отдельно; без любой из улик матч остаётся в знаменателе.
...
(`✅` только при ≥ 99 % и N > 0), запись в `daily.jsonl` (разрезы, топ-10 турниров по пропускам) и нарушители
