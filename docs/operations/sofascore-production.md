...
- Ретенция: при каждом сбросе и `finish` из книги уходят раны без активного claim старше
  3 суток (`ALLOCATION_RUN_RETENTION_SECONDS`), а сверх 300 ранов (`ALLOCATION_RUN_CAP`) —
  самые старые неактивные. Ран с активным claim не удаляется никогда. Число удалённых —
  в логе шлюза («allocation ledger compacted»). Ключ удалённого рана остаётся в
  `retired_runs` (≈100 байт на ран): подписанный план срока не имеет, и поздний claim по
  такому рану получает отказ «DagRun plan was retired», а не новую нулевую аллокацию. Замер на копии книги истории 23.09:
  1 480 → 300 ранов, 29,0 → 2,4 МБ, запись 18,8 мс (медиана).
...
Без bearer-токена аренды `stats` отвечает 401 из того же цикла событий — для задержки этого
достаточно. Приёмка #1350: p95 обоих < 50 мс при идущем батче, `ReadTimeout` в логах задач
0/сутки.
