...
- **Когда всё-таки красный.** Отказаны все строки таблицы (или все матчи пачки) на свежих данных —
  это дефект парсера/контракта: ошибка `all N rows of <table> rejected: <code>` в `errors[]`. Красный
