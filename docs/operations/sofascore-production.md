...
- Закрыт — `min(_ingested_at)` строки `bronze.sofascore_match_capture_status`, где все пять `*_status` ∈
  {`success`, `not_available`} (`not_available` = 404/пусто у источника, терминально; `adapters._legacy_state`).
- «Поздно размечено» — `finished` увиден позже 24 ч после старта: в критерий не входит, считается отдельно.
...
ещё не отчитанные. Итог — одна строка сводки
...
Повторный запуск за тот же D печатает сохранённую строку; `--dry-run` ничего не пишет. Сбой — строка
`⚠️ измеритель вехи 1: <причина>`, сводка не падает.

**Ограничения.** Гранулярность первого наблюдения — сутки (observe только из сводки): матч, который шёл во время
утреннего прогона, будет увиден через ~24 ч и может попасть в «поздно размечено». Первый прогон размечает весь
7-суточный хвост «поздно» — честные числа начинаются со вторых суток журнала. До доставки релиза с
...
(C1 22.09 — 3 074), ≤ 24 ч 186, ядро 136/2 755 по обоим правилам.
