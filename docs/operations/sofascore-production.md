...
- **Манифест на скоуп одним запросом.** `TrinoManifestStore.preload_scope(tournament, season)` читает
  ...
  дописывают индекс, запись как была — пачками по `SOFASCORE_MANIFEST_BATCH_SIZE`. Preload делает
  каждый экземпляр хранилища фазы: план (`prepare_workload_plan`), захват матчей
  (`_run_match_capture`) и сезонный раннер (`_run_season_capture_engine` — фаза `season` истории и
  дейли). Раньше план спрашивал манифест на каждый эндпоинт каждого
...
- **`trino_queries` в отчёте фазы.** Модуль `scrapers/sofascore/trino_accounting.py` считает каждый
  ...
  скоупа — `refresh-results/*.json` (актуалка) и `results/*.json` (история). После последнего
  сброса буфера в `main` поле в уже записанном отчёте обновляется, так что и ошибочный исход
  учитывает этот сброс. Виды: `select` — чтения
  (preload, точечные чтения вне скоупа, пробы расписания, проверка соединения); `merge` — MERGE
  пачек манифеста; `other` — DDL и служебные операторы пакетной записи, включая её проверочный
  `SELECT count(*)` (он часть записи, не чтение). Ожидание на скоуп: `select` ≤ 10 при любом объёме
  записи; `merge` = числу сброшенных пачек (≤ `SOFASCORE_MANIFEST_BATCH_SIZE` записей в пачке).
