...
первый deploy запустить с `--keep-paused` и сохранить отдельный
`backfill-deployment.json`; все три isolated DAG должны остаться paused. Затем
...
python scripts/fotmob_backfill.py run \
  ...
  --env-file /protected/path/fotmob.env \
  --deployment-report /durable/path/fotmob-evidence/backfill-deployment.json \
...
отчётов выполнить обычный deploy без `--keep-paused`, сохранить
финальный `deployment.json` и ждать schedule.
...
2. Развернуть reviewed immutable commit, который возвращает consumers на
   frozen legacy Bronze, обязательно с `deploy.py --keep-paused` и отдельным
   `--report .../rollback-deployment.json`. Откат SQL должен быть выполнен до
...
3. Создать synthetic publication generation и запустить ровно один fenced
   ...
   python scripts/fotmob_rollback.py run-silver \
     --env-file /protected/path/fotmob.env \
     --deployment-report /durable/path/fotmob-evidence/rollback-deployment.json \
...
Validate принимает только Silver run со `start_date` не раньше
`rollback-deployment.json.generated_at`, проверяет отсутствие и `running`, и
...
нулевых active runs isolated stack: сначала pause isolated, затем установить в
shared Variable `fotmob_schedule_owner=shared`. После устранения инцидента
