...
Source refresh запрещён до формального первого replay. Только если этот replay
доказал ровно семь отсутствующих player raw/manifest inputs и их ID совпали с
...
выполнить отдельный bounded запуск. Он не читает catalog, scope или squad и не
