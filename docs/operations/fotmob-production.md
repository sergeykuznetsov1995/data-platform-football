...
> **Какой документ читать.** Здесь описан **церемониальный** путь доставки
> (canary → activation → evidence-цепочка). Живой изолированный контур с 2026-08-11
> работает **без церемонии** — доставка идёт подменой смонтированного дерева, а
> `deployment.json`, canary и шаг `activate` в нём не существуют. Для эксплуатации,
> приёмки деплоя и отката этого контура смотрите
> [`fotmob-isolated-ceremony-free.md`](fotmob-isolated-ceremony-free.md).
> Описание установившегося состояния ниже (таблица шести DAG) верно для обоих путей.
