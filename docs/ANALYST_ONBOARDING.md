...
2. **macOS: один файл-резолвер** (клиент Tailscale на macOS не применяет
   раздаваемый сервером split-DNS для доп. домена — известный баг; проверка:
   `scutil --dns | grep sk-vpn` покажет только `tail.sk-vpn`). Направляем весь
   домен в VPN-резолвер — там уже все адреса (VPN + публичные) из extra_records:
   ```bash
   sudo mkdir -p /etc/resolver
   echo "nameserver 100.100.100.100" | sudo tee /etc/resolver/sk-vpn-2026.uk
   sudo dscacheutil -flushcache; sudo killall -HUP mDNSResponder
   ```
   Проверка: `dscacheutil -q host -a name trino.sk-vpn-2026.uk` → `100.64.0.1`.
   На Linux/Windows split-DNS обычно применяется сам — файл не нужен.
